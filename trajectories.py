...
def retrieve_trajectories(patient_id,all_trajectories_info):
	...
	for trajectory_info in all_trajectories_info:
		if str(trajectory_info[5]) == str(patient_id):
			...
			event_list.append(Event(the_value+"_B", month_diff(str(trajectory_info[0]),first_date_ever),100))
			event_list.append(Event(the_value+"_E", month_diff(str(trajectory_info[1]),first_date_ever),100))
...
def get_all_trajectories(cohort_query):
	extended_query = "select from_date,to_date,value,category,entity,id_patient from activities where entity = 'treatment' and type = 'pec' and id_patient in ("+cohort_query[:-1]+") order by id_patient, to_date"
...
def represent_cohort(cohort_query, density_threshold, func_action="represent"):
	...
	for patient_info in patients_info: 
		...
		member_cnt += 1
		patient_trajectory = retrieve_trajectories(patient_id,all_trajectories_info)
		cohort_trajectories[patient_id]= patient_trajectory
		cohort_longest_trajectory = update_cohort_longest_trajectory(cohort_longest_trajectory, patient_trajectory)
	end_time = time.time()
	...
	sample_rate = 50.0 / float(member_cnt) # dynamic to obtain only 20 patients for whatever big cohort
	# sample_rate = 0
	for i in range(0,member_cnt):
		for j in range (i+1,member_cnt):
			to_pick = random.random()
			if to_pick <= sample_rate:
				match_score, match_matrix = sequence_matching.compute_needleman_wunsch(cohort_trajectories[cohort_members[i]],cohort_trajectories[cohort_members[j]],"score_seq")
	...
	logging.info("matched trajectories in "+str(time_length)+" seconds (made "+str(cnt_scores)+" comparisons).")
	...
	for match_matrix_action in match_matrix_actions:
		if match_matrix_action != "" and match_matrix_action != None:
			...
			new_behavior = CohortEvent(event_name,abs(int(event_month1)-int(event_month2)))
			cohort_behavior[which_cell].append(new_behavior)
	max_frequency = 1 # maximum number of times an action is repeated in the cohort.
	final_rep = ""
	virtual_patient = []
	for cell in cohort_behavior:
		...
		if len(cohort_behavior[cell]) != 0:
			...
			for unique_action in set_notion:
				ratio = float(frequency[unique_action]) / float(max_frequency) * 100
				if ratio >= density_threshold:
					final_rep += "t"+str(cell)+":" + str(unique_action) + " ("+str(ratio)[0:5]+"%)\n"
...
def compare_cohort_pair(cohort1_query, cohort2_query, density_threshold, action="score"):
	virutal_trajectory1 = represent_cohort(cohort1_query, density_threshold, "virtual")
	virutal_trajectory2 = represent_cohort(cohort2_query, density_threshold, "virtual")
	...
	start_time = time.time()
	match_score, match_matrix = sequence_matching.compute_needleman_wunsch(virutal_trajectory1,virutal_trajectory2,"score_seq")
	end_time = time.time()
	time_length = end_time - start_time
	...
	for match_matrix_action in match_matrix_actions:
		if match_matrix_action != "" and match_matrix_action != None:
			...
			event_name = parts[0]
			which_cell = abs(int(event_month1)+int(event_month2))/2
			how_good = float(longest_trajectory - abs(int(event_month1)-int(event_month2))) / float(longest_trajectory) * 100.0
		...
		if len(cohort_behavior[cell]) != 0:
			for action in cohort_behavior[cell]:
				if action.confidence >= density_threshold:
					final_rep += "t"+str(cell)+":" + str(action) + " ("+str(action.confidence)[0:5]+"%)\n"
					virtual_patient.append(Event(action.name,cell,action.confidence))
	if action == "represent":
		return final_rep
	return virtual_patient
