...
def fill_similarity_matrix(patient1_trajectory, patient2_trajectory, largest_trajectory_length, longest_trajectory_length):
	...
	for i in range(0,largest_trajectory_length+2):
		similarity_matrix[i] = {}
		similarity_direction[i] = {} 
		for j in range(0,largest_trajectory_length+2):
			if i == 0:
			...
			if i != 0 and j != 0 :
				...
				choice2 = similarity_matrix[i-1][j] + indel_score
				choice3 = similarity_matrix[i][j-1] + indel_score
...
def output_similarity_matrix_html(patient1_trajectory, patient2_trajectory, largest_trajectory_length, longest_trajectory_length, similarity_matrix, similarity_direction):
	html_file = "nw_html.html"
	file = open(html_file,"w")
	similarity_matrix_out = "<head><link rel='stylesheet' href='table.css'></head>"
	similarity_matrix_out += "<p>"+str(patient1_trajectory)+"</p>"
	similarity_matrix_out += "<p>"+str(patient2_trajectory)+"</p>"
	similarity_matrix_out += "<table border=1><tr><td></td><td></td>"
	for event in patient2_trajectory:
		similarity_matrix_out += "<td>"+event.name+" (t"+str(event.month)+")</td>"
	for i in range(1,largest_trajectory_length-len(patient2_trajectory)):
		similarity_matrix_out += "<td>-</td>"
	similarity_matrix_out += "</tr><tr>"
	for i in range(0,largest_trajectory_length+2):
		if i == 0:
			similarity_matrix_out += "<td></td>"
		else:
			try:
				similarity_matrix_out += "<td>"+patient1_trajectory[i-1].name+" (t"+str(patient1_trajectory[i-1].month)+")</td>"
			except:
				similarity_matrix_out += "<td>-</td>"
		for j in range(0,largest_trajectory_length):
			similarity_matrix_out += "<td class='row'><img src='"+similarity_direction[i][j]+".png' width='20' height='20'/>"+str(similarity_matrix[i][j])+"</td>"
		similarity_matrix_out += "</tr>"
	similarity_matrix_out += "</table>"
	...
	similarity_matrix_out += "<p>"+str(common_sequence)+"</p>"
	file.write(similarity_matrix_out)
	file.close()
...
def compute_needleman_wunsch(patient1,patient2,action="score"):
	...
	if action == "matrix":
		...
		if action == "score":
			return score
		if action == "score_seq":
			print output_similarity_matrix_html(patient1_trajectory,patient2_trajectory,largest_trajectory_length, longest_trajectory_length,similarity_matrix,similarity_direction)
